  ...
  <select id="{{ $field['id'] }}" name="{{ $field['name'] }}" class="{{ $field['controlClass'] }}" @required($field['required'])>
    ...
    @if (isset($formSubmission) && isset($formSubmission->data[$field['name']]))
      ...
      <option value="{{ $formSubmission->data[$field['name']] }}" selected>{{ $formSubmission->data[$field['name']] }}
        {{ $formSubmission->data[$field['name']] }} - {{ $patrimonio['epfmarpat'] }} - {{ $patrimonio['tippat'] }} - {{ $patrimonio['modpat'] }}</option>
    @elseif ($field['old'])
      @php
        $patrimonio =  \Uspdev\Replicado\Bempatrimoniado::dump($field['old']);
      @endphp
      <option value="{{ $field['old'] }}" selected>
        {{ $field['old'] }} - {{ $patrimonio['epfmarpat'] }} - {{ $patrimonio['tippat'] }} - {{ $patrimonio['modpat'] }}
      </option>
    @endif
  </select>
